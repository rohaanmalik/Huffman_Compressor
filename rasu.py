...
def huffman_tree(freq_dict):
    ...
    Special Case: When the length of the dictionary is 1, the sibling of the \
    leaf (having value as the  key of the dictionary ) will have the dummy vaue\
     '*' as arity of a Huffman Tree is always2
    @param dict(int,int) freq_dict: a frequency dictionary
    @rtype: HuffmanNode

    >>> freq = {2: 6, 3: 4, 5: 5, 6: 6}
    >>> t = huffman_tree({0: 1, 1: 1})
    >>> t == HuffmanNode(None, HuffmanNode(0, None, None), \
    HuffmanNode(1, None, None))
    True
    >>> freq1 = {1: 0}
    >>> t = huffman_tree(freq1)
    >>> t
    HuffmanNode(None, HuffmanNode(1, None, None), HuffmanNode(*, None, None))
    """
    if len(freq_dict) == 1:
        for key in freq_dict:
            node = HuffmanNode(None, HuffmanNode(key), HuffmanNode('*'))
        return node
    ...
    while len(node_list) != 1:
        ...
        first, second = node_list.pop(0), node_list.pop(0)

        if first.number <= second.number:
            root = HuffmanNode(None, first, second)
        else:
            root = HuffmanNode(None, second, first)

        root.number = first.number + second.number
...
def generate_tree_general(node_lst, root_index):
    ...
    HuffmanNode(None, HuffmanNode(3, None, None), HuffmanNode(4, None, None))),\
     HuffmanNode(None, HuffmanNode(5, None, None), HuffmanNode(6, None, None)))
    """
    return function1(HuffmanNode(None, HuffmanNode(None), HuffmanNode(None)),
